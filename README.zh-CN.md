...
日常操作建议先跑 `status` 看 checkout、runtime binary 和 active Store，再用
`doctor` 做本地依赖与配置诊断；干净机器可以用 `setup --build-runtime` 或
`doctor --fix` 补齐本地 Store 和 runtime 目录；需要更新到最新 tag 运行时时用
`update --channel release` 或 `update --release latest`；不确定命令路径时用
`commands --area AREA --filter TEXT` 搜索。`config show`、`config path`、
`logs` 和 `completion bash|zsh` 分别用于查看本地配置、定位配置文件、读取 runtime
日志和接入 shell completion。
