...
| Suite and interface reports / 集合与接口报告 | Runs exact case ids, cases selected by maintenance metadata, or all cases attached to one interface node, then returns JSON plus a temporary HTML report. Failed cases remain part of the report. 按精确用例 ID、维护条件选择用例，或执行某个接口节点下的全部用例，返回 JSON 和临时 HTML 报告；失败用例保留在报告内。 | `case suite report`, `interface-node case report`, `/api/cases/batch-runs` |
