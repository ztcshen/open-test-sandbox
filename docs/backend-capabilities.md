...
| Feature research / Feature 研究 | Consumes the external GitHub Feature Radar index, checks index freshness before use, audits reference and project-index policy locally, lists and filters feature records, gates the full feature index and individual CLI design slices on enough recent 3K+ star open-source references, explains feature-to-reference matrices with project-index metadata, builds refresh plans from stale/audit/coverage signals, ranks roadmap candidates by reference coverage and command availability, turns them into a stateless implementation backlog, and can produce a one-shot research plan with catalog-verified next commands and verification commands without bundling crawler data into core. `research gate` adds a non-zero pre-implementation guard that combines freshness, audit, feature reference coverage, and optional required command-path checks for the CLI slice being changed. 读取外部 GitHub Feature Radar 索引，在使用前检查索引新鲜度，在本地审计引用和项目索引策略，列出和过滤 feature 记录，用足够的近期活跃 3K+ star 开源参考为整体 feature 索引和单个 CLI 设计切片做门禁，用项目索引元数据解释 feature 到参考项目的矩阵，并从过期、审计和覆盖信号生成刷新计划，按引用覆盖和命令可用性排序 roadmap 候选，把候选转成无状态实现 backlog，并可生成包含命令目录校验 next commands 与 verification commands 的一次性研究计划，同时不把爬虫数据打包进核心；`research gate` 为正在修改的 CLI 切片增加 freshness、audit、feature 引用覆盖和可选 command-path 校验的非零前置门禁。 | `research features`, `research feature`, `research status`, `research audit`, `research coverage`, `research matrix`, `research refresh-plan`, `research roadmap`, `research backlog`, `research gate`, `research plan`, `docs/feature-research.md` |
