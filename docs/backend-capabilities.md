...
| --- | --- | --- |
| CLI command catalog and operator diagnostics / 命令目录与操作诊断 | Exposes a searchable machine-readable command catalog derived from the same Usage source as the help screen, plus clean-machine setup, status, doctor, update, local config, log, and shell completion commands. Agents and automation can discover command paths, areas, Store awareness, tags, and concrete next actions before executing. 暴露从 help 同源 Usage 解析出的可搜索机器可读命令目录，并提供干净机器 setup、status、doctor、update、本地配置、日志和 shell completion 命令；agent 和自动化可在执行前发现命令路径、领域、Store 感知、标签和具体下一步动作。 | `setup`, `status`, `doctor`, `update`, `commands`, `config show`, `logs`, `completion`, `help` |
...
| --- | --- |
| General diagnostics / 基础诊断 | `setup`, `status`, `doctor --fix`, `doctor --deep`, `update --channel main|release`, `commands --area`, `config show`, `logs`, `completion` |
