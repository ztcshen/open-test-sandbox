...
| Case maintenance / 用例维护 | Indexes case description, tags, priority, owner, status, runnable file presence, and execution configuration for review, assignment, coverage, and suite execution. CLI reports, Control plane coverage, and batch rerun selectors share one backend rule module. 索引用例说明、标签、优先级、owner、状态、可运行文件和执行配置，便于审阅、分派、覆盖检查和集合执行；CLI 报告、控制平面覆盖率和批量重跑选择器共用同一套后端规则模块。 | `internal/casesuite`, `case discover`, `case suite coverage`, `case suite report`, Store catalog, profile API case metadata |
