...
| `GET` | `/api/environments/{environmentId}/bootstrap` | Return the local clone/fetch, compose/start, component graph restore-readiness, health-check, and verification workflow plan for the environment. This is a plan surface only; local CLI restore executes Git, Docker, health checks, and workflow runs. |
